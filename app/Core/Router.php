...
class Router
{
    ...
    public function get(string $u, callable|array $a, ...$mws)
    {
        $this->add('GET', $u, $a, $mws);
    }
    public function post(string $u, callable|array $a, ...$mws)
    {
        $this->add('POST', $u, $a, $mws);
    }
    public function put(string $u, callable|array $a, ...$mws)
    {
        $this->add('PUT', $u, $a, $mws);
    }
    public function patch(string $u, callable|array $a, ...$mws)
    {
        $this->add('PATCH', $u, $a, $mws);
    }
    public function delete(string $u, callable|array $a, ...$mws)
    {
        $this->add('DELETE', $u, $a, $mws);
    }
    ...
    private function callAction(callable|array $action, array $routeParams = []): Response
    {
        ...
        if (!$result instanceof Response) {
            if (is_string($result)) {
                // Wrap string in array for Response
                return new Response(['message' => $result], 200);
            }
            // If it's null or something else, wrap it or throw
