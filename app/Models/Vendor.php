...
class Vendor
{
    ...
    {
        $stmt = $this->db->query("SELECT id, email, name, phone, address, food_types, rating, image FROM vendor");
    ...
    {
        $stmt = $this->db->prepare("SELECT id, email, name, phone, address, food_types, rating, image FROM vendor WHERE id = :id");
        $stmt->execute(['id' => $id]);
        $result = $stmt->fetch();
