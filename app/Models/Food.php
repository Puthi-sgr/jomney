...
class Food
{
    ...
    {
        $sql = "SELECT 
            f.id, f.vendor_id, f.name, f.description, f.category, f.price, f.ready_time, f.rating, f.image,
            i.qty_available
            FROM food f
        ...
        if (!empty($filters)) {
            if (isset($filters['vendor_id'])) {
                $conditions[] = "f.vendor_id = :vendor_id";
    ...
    public function allWithVendorInfo(array $filters = []): array
    {
        $sql = "SELECT
                f.id, f.vendor_id, f.name, f.description, f.category, f.price, f.ready_time, f.rating, f.image,
                i.qty_available,
    ...
    public function allByVendorIds(array $vendorIds): array
    {
        if (empty($vendorIds)) {
            return [];
        }
        ...
        $sql = "SELECT 
            ...
            FROM food f
            JOIN inventory i ON f.id = i.food_id
            WHERE f.vendor_id IN ($placeholders)
            ORDER BY f.vendor_id, f.name";
        $stmt = $this->db->prepare($sql);
